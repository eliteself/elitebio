...
public struct BiometricAuthView: View {
    ...
    public var body: some View {
        VStack(spacing: 24) {
            ...
            // Title
            Text("Welcome to EliteNet")
                .font(.largeTitle)
                .fontWeight(.bold)
                .multilineTextAlignment(.center)
            
            // Subtitle
            Text("Use \(biometricManager.biometricType.displayName) to sign in")
                .font(.title3)
            ...
            VStack(spacing: 16) {
                ...
                Button(action: authenticate) {
                    HStack {
                        ...
                        Text(isAuthenticating ? "Authenticating..." : "Sign In with \(biometricManager.biometricType.displayName)")
                    }
        ...
        .alert("Error", isPresented: $showErrorAlert) {
            if let action = errorAction {
                switch action {
                case .showAlert(let title, _, let primaryAction, let secondaryAction):
    ...
    private func handleAuthStateChange(_ state: BiometricAuthState) {
        ...
        case .authenticated:
            onSuccess()
        case .failed(let message):
            handleError(BiometricError.authenticationFailed(message))
        case .notAvailable(let message):
            handleError(BiometricError.notAvailable)
        case .lockedOut(let message):
            handleError(BiometricError.lockedOut(0))
        default:
            break
        }
    }
    
    private func handleError(_ error: BiometricError) {
        currentError = error
    ...
    private func handlePrimaryAction() {
        ...
        if let error = currentError, BiometricErrorHandler.canRetry(error) {
            ...
            onFailure(currentError ?? BiometricError.authenticationFailed("Unknown error"))
        }
    }
    ...
    public func body(content: Content) -> some View {
        content
            ...
            .sheet(isPresented: $showAuth) {
                BiometricAuthView(config: config) {
                    showAuth = false
                    onSuccess()
                } onFailure: { error in
                    // Handle failure - could show alert or retry
...
public extension View {
    func biometricAuth(
        ...
        modifier(BiometricAuthModifier(config: config, onSuccess: onSuccess))
    }
}

// MARK: - Biometric Auth Manager (Singleton)
public class BiometricAuthManager: ObservableObject {
    ...
    public func authenticate(config: BiometricConfig = .default) async throws -> Bool {
        return try await biometricManager.authenticate(config: config)
    }
